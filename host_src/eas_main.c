...
static EAS_RESULT PlayFile (EAS_DATA_HANDLE easData, const char* filename, const char* outputFile, const S_EAS_LIB_CONFIG *pLibConfig, void *buffer, EAS_I32 bufferSize)
{
    ...
    if (outputFile == NULL)
    {
        ...
        {
            EAS_ReportX(_EAS_SEVERITY_ERROR, "Error in output filename %s\n", waveFilename);
            return EAS_FAILURE;
        }
    ...
    {
        EAS_ReportX(_EAS_SEVERITY_ERROR, "EAS_OpenFile returned %ld\n", reportResult);
        return reportResult;
    }
    ...
    {
        EAS_ReportX(_EAS_SEVERITY_ERROR, "EAS_Prepare returned %ld\n", result);
        reportResult = result;
    }
    ...
    {
        EAS_ReportX(_EAS_SEVERITY_ERROR, "EAS_ParseMetaData returned %ld\n", result);
        return result;
    }
    EAS_ReportX(_EAS_SEVERITY_NOFILTER, "Play length: %d.%03d (secs)\n" , playTime / 1000, playTime % 1000);
    ...
    if (reportResult == EAS_SUCCESS)
    {
        ...
        if (!wFile)
        {
            EAS_ReportX(_EAS_SEVERITY_ERROR, "Unable to create output file %s\n", waveFilename);
    ...
    while (reportResult == EAS_SUCCESS)
    {
        ...
        for (i = 0, p = buffer; i < NUM_BUFFERS; i++, p+= pLibConfig->mixBufferSize * pLibConfig->numChannels)
        {
            ...
            {
                EAS_ReportX(_EAS_SEVERITY_ERROR, "EAS_GetLocation returned %d\n",result);
                if (reportResult == EAS_SUCCESS)
                    reportResult = result;
                break;
            }
            EAS_ReportX(_EAS_SEVERITY_DETAIL, "Parser time: %d.%03d\n", playTime / 1000, playTime % 1000);

            /* render a buffer of audio */
            if ((result = EAS_Render(easData, p, pLibConfig->mixBufferSize, &count)) != EAS_SUCCESS)
            {
                EAS_ReportX(_EAS_SEVERITY_ERROR, "EAS_Render returned %d\n",result);
                if (reportResult == EAS_SUCCESS)
                    reportResult = result;
            }
        }
        ...
        if (result == EAS_SUCCESS)
        {
            ...
            {
                EAS_ReportX(_EAS_SEVERITY_ERROR, "WaveFileWrite failed\n");
                reportResult = EAS_FAILURE;
            }
        }
        ...
        if (reportResult == EAS_SUCCESS)
        {
            ...
            {
                EAS_ReportX(_EAS_SEVERITY_ERROR, "EAS_State returned %d\n", result);
                reportResult = result;
            }
    ...
    if (wFile)
    {
        ...
        {
            EAS_ReportX(_EAS_SEVERITY_ERROR, "Error closing wave file %s\n", waveFilename);
            if (reportResult == EAS_SUCCESS)
                result = EAS_FAILURE;
        }
    }
    ...
    {
        EAS_ReportX(_EAS_SEVERITY_ERROR, "EAS_Close returned %ld\n", result);
        if (reportResult == EAS_SUCCESS)
            result = EAS_FAILURE;
    }
...
int main( int argc, char **argv )
{
        ...
        if (argv[i][0] == '-')
        {
            ...
            case 'd':
                ...
                if ((temp >= '0') || (temp <= '9'))
                    ...
                    EAS_ReportX(_EAS_SEVERITY_WARNING, "Invalid debug level %d\n", temp);
                break;
            case 'f':
                if ((debugFile = fopen(&argv[i][2],"w")) == NULL)
                    EAS_ReportX(_EAS_SEVERITY_WARNING, "Unable to create debug file %s\n", &argv[i][2]);
                else
            ...
            case 'p':
                ...
                if (polyphony < 1)
                    polyphony = 1;
                EAS_ReportX(_EAS_SEVERITY_INFO, "Polyphony set to %d\n", polyphony);
                break;
            default:
                EAS_ReportX(_EAS_SEVERITY_INFO, "Unknown option: \"%s\"\n\n", argv[i]);
                printHelp();
                break;
            }
            continue;
    ...
    if (!buffer)
    {
        EAS_ReportX(_EAS_SEVERITY_FATAL, "Error allocating memory for audio buffer\n"); 
        return EAS_FAILURE;
    }
    ...
    {
        EAS_ReportX(_EAS_SEVERITY_FATAL, "EAS_Init returned %ld - aborting!\n", result); 
        free(buffer);
        return result;
    }
    ...
    if (argc < 2)
    {
        EAS_ReportX(_EAS_SEVERITY_INFO, "Playing '%s'\n", defaultTestFile);
    ...
    {
        EAS_ReportX(_EAS_SEVERITY_FATAL, "EAS_Shutdown returned %ld\n", result);
    }
...
static EAS_BOOL EASLibraryCheck (const S_EAS_LIB_CONFIG *pLibConfig)
{
    ...
    EAS_ReportX(_EAS_SEVERITY_INFO, "EAS Library Version %d.%d.%d.%d\n",
    ...
    if (pLibConfig->checkedVersion)
        EAS_ReportX(_EAS_SEVERITY_INFO, "\tChecked library\n");
    EAS_ReportX(_EAS_SEVERITY_INFO, "\tMaximum polyphony: %d\n", pLibConfig->maxVoices);
    EAS_ReportX(_EAS_SEVERITY_INFO, "\tNumber of channels: %d\n", pLibConfig->numChannels);
    EAS_ReportX(_EAS_SEVERITY_INFO, "\tSample rate: %d\n", pLibConfig->sampleRate);
    EAS_ReportX(_EAS_SEVERITY_INFO, "\tMix buffer size: %d\n", pLibConfig->mixBufferSize);
    if (pLibConfig->filterEnabled)
        EAS_ReportX(_EAS_SEVERITY_INFO, "\tFilter enabled\n");
#ifndef _WIN32_WCE
    EAS_ReportX(_EAS_SEVERITY_INFO, "\tLibrary Build Timestamp: %s", ctime((time_t*)&pLibConfig->buildTimeStamp));
#endif
    EAS_ReportX(_EAS_SEVERITY_INFO, "\tLibrary Build ID: %s\n", pLibConfig->buildGUID);
    ...
    {
        EAS_ReportX(_EAS_SEVERITY_FATAL, "Library version does not match header files. EAS Header Version %d.%d.%d.%d\n",
            LIB_VERSION >> 24,
            (LIB_VERSION >> 16) & 0x0f,
            (LIB_VERSION >> 8) & 0x0f,
            LIB_VERSION & 0x0f);
